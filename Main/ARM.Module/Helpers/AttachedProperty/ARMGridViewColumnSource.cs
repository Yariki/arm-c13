...
namespace ARM.Module.Helpers.AttachedProperty
{
    public static class ARMGridViewColumnSource
    {
        public static readonly DependencyProperty ColumnsSourceProperty = DependencyProperty.RegisterAttached(
            "ColumnsSource", typeof(ObservableCollection<GridViewColumn>), typeof(ARMGridViewColumnSource), new PropertyMetadata(default(ObservableCollection<GridViewColumn>), ColumnsSourceChanged));
        ...
        private static void ColumnsSourceChanged(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs args)
        {
            ...
            var columns = args.NewValue as ObservableCollection<GridViewColumn>;
            if (columns != null)
            {
                var selector = (DataTemplateSelector)gridView.GetValue(ARMGridViewColumnSource.CertificationCellTemplateSelectorProperty);
                columns.ForEach(col =>
                {
                    col.CellTemplateSelector = selector;
                    gridView.Columns.Add(col);
                });
            }
        }

        public static void SetColumnsSource(DependencyObject element, ObservableCollection<GridViewColumn> value)
        {
            element.SetValue(ColumnsSourceProperty, value);
        }

        public static ObservableCollection<GridViewColumn> GetColumnsSource(DependencyObject element)
        {
            return (ObservableCollection<GridViewColumn>)element.GetValue(ColumnsSourceProperty);
        }
